...
`generateContent` 実行時は、ハードコードされたプロンプトではなく、**Supabase (`prompts` テーブル)** から取得した動的プロンプトを使用します。
- **systemInstruction**: 
    - Supabaseから最新の指示を取得。取得失敗時はデフォルトの「Mr.OWL」プロンプトを使用。
...
### 4.1 Ritual Components
没入感と心理的区切りを提供するため、全画面オーバーレイとアニメーションを多用します。

- **Morning Ritual**:
  - `bg-[#E0EAFC]` (Morning Haze)
  - `animate-in fade-in zoom-in` (Slow Awakening)
- **Closing Ritual**:
  - `bg-slate-900` (Night Sky)
  - `animate-stamp` (Visceral Completion)
  - **"It's All Right"** typography: `font-thin tracking-widest`
